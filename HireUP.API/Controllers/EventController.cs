...
namespace HireUP.API.Controllers
{
    ...
    public class EventController : ControllerBase
    {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> RegisterEmployer(int eventId, [FromBody] RegisterEmployerDto dto)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                await _eventService.RegisterEmployerInEvent(eventId, dto.EmployerId);
        ...
        [HttpDelete("{eventId}/unregister/{employerId}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> UnregisterEmployer(int eventId, int employerId)
        {
            try
            {
                await _eventService.UnregisterEmployerFromEvent(eventId, employerId);
